...
public class Login extends HttpServlet
{
            ...
            throws ServletException, IOException
    {
        ...
        Object user = userSrv.findUserByNo(name).get(0);
        ...
        if(name.equals("") || pass.equals(""))
        {
            ...
            if(((User) user).getEmp_type() == 1 && ((User) user).getEmp_no().equals(name)
                    && ((User) user).getEmp_pass().equals(pass))
            {
                ...
                if(((User) user).getEmp_type() == 0 && ((User) user).getEmp_no().equals(name)
                        && ((User) user).getEmp_pass().equals(pass))
                {
                    request.setAttribute("name", name);
                    request.getSession().setAttribute("login", "ok");
